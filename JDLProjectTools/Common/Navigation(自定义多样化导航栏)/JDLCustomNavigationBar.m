...
#define kJDLDefaultTitleSize 18
#define kJDLDefaultTitleColor [UIColor blackColor]
#define kJDLDefaultBackgroundColor [UIColor whiteColor]
#define kJDLScreenWidth [UIScreen mainScreen].bounds.size.width

@implementation UIViewController (JDLRoute)

- (void)JDL_toLastViewController
{
...
+ (UIViewController*)JDL_currentViewController
{
    UIViewController* rootViewController = [UIApplication sharedApplication].delegate.window.rootViewController;
    return [self JDL_currentViewControllerFrom:rootViewController];
}

+ (UIViewController*)JDL_currentViewControllerFrom:(UIViewController*)viewController
{
    if ([viewController isKindOfClass:[UINavigationController class]]) {
        UINavigationController* navigationController = (UINavigationController *)viewController;
        return [self JDL_currentViewControllerFrom:navigationController.viewControllers.lastObject];
    }
    else if([viewController isKindOfClass:[UITabBarController class]]) {
        UITabBarController* tabBarController = (UITabBarController *)viewController;
        return [self JDL_currentViewControllerFrom:tabBarController.selectedViewController];
    }
    else if (viewController.presentedViewController != nil) {
        return [self JDL_currentViewControllerFrom:viewController.presentedViewController];
    }
    else {
...
@interface JDLCustomNavigationBar ()
...
@implementation JDLCustomNavigationBar

+ (instancetype)CustomNavigationBar {
    JDLCustomNavigationBar *navigationBar = [[self alloc] initWithFrame:CGRectMake(0, 0, kJDLScreenWidth, [JDLCustomNavigationBar navBarBottom])];
...
-(void)setupView
{
    ...
    self.backgroundView.backgroundColor = kJDLDefaultBackgroundColor;
}

// TODO:这边结合 JDLCellView 会不会更好呢？
-(void)updateFrame
{
    NSInteger top = ([JDLCustomNavigationBar isIphoneX]) ? 44 : 20;
    NSInteger margin = 0;
    ...
    self.rightButton.frame = CGRectMake(kJDLScreenWidth - buttonWidth - margin, top, buttonWidth, buttonHeight);
    self.titleLable.frame = CGRectMake((kJDLScreenWidth - titleLabelWidth) / 2, top, titleLabelWidth, titleLabelHeight);
    self.bottomLine.frame = CGRectMake(0, (CGFloat)(self.bounds.size.height-0.5), kJDLScreenWidth, 0.5);
}
...
-(void)clickBack {
    if (self.onClickLeftButton) {
        ...
        UIViewController *currentVC = [UIViewController JDL_currentViewController];
        [currentVC JDL_toLastViewController];
    }
}
-(void)clickRight {
...
- (void)JDL_setLeftButtonWithImage:(UIImage *)image title:(NSString *)title titleColor:(UIColor *)titleColor {
    [self JDL_setLeftButtonWithNormal:image highlighted:image title:title titleColor:titleColor];
}
- (void)JDL_setLeftButtonWithNormal:(UIImage *)normal highlighted:(UIImage *)highlighted {
    [self JDL_setLeftButtonWithNormal:normal highlighted:highlighted title:nil titleColor:nil];
}
- (void)JDL_setLeftButtonWithImage:(UIImage *)image {
    [self JDL_setLeftButtonWithNormal:image highlighted:image title:nil titleColor:nil];
}
- (void)JDL_setLeftButtonWithTitle:(NSString *)title titleColor:(UIColor *)titleColor {
    [self JDL_setLeftButtonWithNormal:nil highlighted:nil title:title titleColor:titleColor];
}
...
- (void)JDL_setRightButtonWithImage:(UIImage *)image title:(NSString *)title titleColor:(UIColor *)titleColor {
    [self JDL_setRightButtonWithNormal:image highlighted:image title:title titleColor:titleColor];
}
- (void)JDL_setRightButtonWithNormal:(UIImage *)normal highlighted:(UIImage *)highlighted {
    [self JDL_setRightButtonWithNormal:normal highlighted:highlighted title:nil titleColor:nil];
}
- (void)JDL_setRightButtonWithImage:(UIImage *)image {
    [self JDL_setRightButtonWithNormal:image highlighted:image title:nil titleColor:nil];
}
- (void)JDL_setRightButtonWithTitle:(NSString *)title titleColor:(UIColor *)titleColor {
    [self JDL_setRightButtonWithNormal:nil highlighted:nil title:title titleColor:titleColor];
}
...
-(UILabel *)titleLable {
    if (!_titleLable) {
        ...
        _titleLable.textColor = kJDLDefaultTitleColor;
        _titleLable.font = [UIFont systemFontOfSize:kJDLDefaultTitleSize];
...
+ (int)navBarBottom
{
    if ([JDLCustomNavigationBar isIphoneX]) {
        return 88;
    } else {
        return 64;
    }
