...
@interface JDLPageContentScrollView () <UIScrollViewDelegate>
// 外界父控制器
...
@implementation JDLPageContentScrollView

- (instancetype)initWithFrame:(CGRect)frame parentVC:(UIViewController *)parentVC childVCs:(NSArray *)childVCs {
    if (self = [super initWithFrame:frame]) {
        if (parentVC == nil) {
            @throw [NSException exceptionWithName:@"JDLPagingView" reason:@"JDLPageContentScrollView 所在控制器必须设置" userInfo:nil];
        }
        self.parentViewController = parentVC;
        if (childVCs == nil) {
            @throw [NSException exceptionWithName:@"JDLPagingView" reason:@"JDLPageContentScrollView 子控制器必须设置" userInfo:nil];
        }
