...
@interface JDLPageContentView () <UICollectionViewDataSource, UICollectionViewDelegate>
/// 外界父控制器
...
@implementation JDLPageContentView

- (instancetype)initWithFrame:(CGRect)frame parentVC:(UIViewController *)parentVC childVCs:(NSArray *)childVCs {
    if (self = [super initWithFrame:frame]) {
        if (parentVC == nil) {
            @throw [NSException exceptionWithName:@"JDLPagingView" reason:@"JDLPageContentView 所在控制器必须设置" userInfo:nil];
        }
        self.parentViewController = parentVC;
        if (childVCs == nil) {
            @throw [NSException exceptionWithName:@"JDLPagingView" reason:@"JDLPageContentView 子控制器必须设置" userInfo:nil];
        }
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    // 3、pageContentViewDelegare; 将 progress／sourceIndex／targetIndex 传递给 JDLPageTitleView
...
#pragma mark - - - 给外界提供的方法，获取 JDLPageTitleView 选中按钮的下标
